...
class Bridge(object):
    def __init__(self, conf, server):
        ...
        self.publishers = {e.name: rospy.Publisher(e.topic, TYPE[e.type],
                                                   queue_size=1)
    ...
    def calc_angular(self, yaw):
        ...
        if self.yaw is not None:
            angular_vel =\
                (yaw - self.yaw) / (rospy.get_time() - self.prev_time)
        self.yaw = yaw
    ...
    def publish_controls(self, data):
        steering, throttle, brake =\
            data['steering_angle'], data['throttle'], data['brake']
    ...
    def publish_traffic(self, data):
        x, y, z =\
            data['light_pos_x'], data['light_pos_y'], data['light_pos_z'],
    ...
    def callback_steering(self, data):
        self.server('steer', data={'steering_angle':
                                   str(data.steering_wheel_angle_cmd)})
